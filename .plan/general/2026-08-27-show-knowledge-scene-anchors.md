...
지식창고 화면에서 지식 항목이 어느 씬·화면에 묶여 있는지 보이게 한다. `anchor` 가 없는 항목은 "게임 전체에서
성립한다"고 분명히 말한다. 목록을 씬 하나로 좁힐 수 있게 한다.

## Non-goals

- 화면에서 `anchor` 를 편집하지 않는다.
...
지식창고에 남는 것 중 한 화면에서만 성립하는 사실은 이제 **`anchor`**(씬 이름 + 선택적 화면 id)를 단다.
`anchor` 가 **없는** 항목은 게임 어디서나 참인 사실이며 이쪽이 보통이다.
...
- 형식이 깨진 `anchor`(`sceneName` 없음)는 그 `anchor` 만 버리고 노드는 살린다.
- `DESIGN.md`: 색만으로 `anchor` 있음/없음을 가르지 않는다. 라벨과 모양을 짝짓는다. box shadow 없다.
...
- [x] **Step 1: 계약** — `knowledgeTypes.ts` 에 `KnowledgeAnchor` 를 두고 `KnowledgeNode.anchors` 를 얹는다.
      `knowledgeApi.ts` 에 `parseKnowledgeAnchor` 를 더한다. 같은 (씬, 화면) `anchor` 가 두 번 오면 하나로 접는다 —
      edge 중복 접기와 같은 이유로, 두 번 그리면 서버가 말하지 않은 두 개의 사실을 만든다.
- [x] **Step 2: 인스펙터** — 고른 항목의 `anchor` 를 `kg-detail-fields` 한 행으로 보인다. `anchor` 가 없으면
...
- [x] **Step 3: 필터** — 목록 위에 씬 `<select>`. `IssuesSection.tsx` 의 `label > span + select` 관용구를 쓴다.
      순수 함수(`knowledgeAnchors.ts`)가 씬 목록과 술어를 낸다. `anchor` 가 하나도 없으면 필터를 아예 그리지 않는다 —
      오늘의 서버에서 쓸모없는 컨트롤이 늘지 않게.
- [x] **Step 4: Tests** — `knowledgeApi.test.ts` 에 `anchor` 파싱 케이스, `knowledgeAnchors.test.ts` 에 필터 케이스.
...
- **Risks:**
  - ARTEL-605 가 다른 모양으로 착지하면(예: `screenId` 를 숫자로, 또는 `anchors` 대신 다른 이름) `anchor` 가
    안 읽힌다. 그때 화면은 전부 "게임 전체"로 읽히고 이는 틀린 말이다 — 실패로 보이지 않는다는 점이 위험이다.
    완화: `screenId` 는 숫자로 와도 문자열로 받아 준다. 필드 이름이 바뀌면 파서 한 줄만 고치면 된다.
  - `anchor` 가 많은 항목은 인스펙터 행이 길어진다. 목록으로 쌓아 세로로만 자란다.
- **Rollback steps:** `git revert`.

## Open Questions

- 목록 행마다 `anchor` 를 표시할지. 지금은 안 한다 — 태그·출처·id 로 이미 붐빈다. 필터가 그 역할을 한다.
