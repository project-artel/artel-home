...
export const projectsEn = {
  ...
  members: {
    ...
    inviteCopy:
      'Type two or more characters to search by nickname, tag, or provider login, or paste the exact email address — a partial address will not match. An address without an ARTEL account can still be invited; the invitation waits for them.',
    inviteTargetLabel: 'Name or email',
    inviteTargetPlaceholder: 'Search by name, or paste an email address',
    candidateListLabel: 'Matching accounts',
    candidateLoading: 'Searching…',
    ...
    sentAnnouncement: (target: string) => `Invitation sent to ${target}.`,
  ...
  apiErrors: {
    ...
    invitation_target_ambiguous: 'The invitation needs exactly one target: a match from the list, or an email address.',
    invitation_target_unreachable: 'That account cannot receive an invitation email.',
  },
  ...
  workspace: {
    ...
    uncovered: {
      ...
      /*
       * 저작 화면에서 이 배지는 **현재 런** 배지와 나란히 놓인다(ARTEL-904). 분모가 같아서
       * 라벨만으로는 무엇을 세는지 갈리지 않으므로, 도움말이 그 선을 그어 준다.
       */
      remainingHelp:
...
export const projectsKo: Localized<typeof projectsEn> = {
  ...
  members: {
    ...
    inviteTitle: '사람 부르기',
    inviteCopy:
      '두 글자 이상 입력하면 nickname, tag, 로그인 이름으로 찾습니다. 이메일 주소는 전체를 그대로 붙여야 찾아지고, 일부만 입력하면 찾지 못합니다. ARTEL 계정이 아직 없는 주소도 부를 수 있고, 그 사람이 가입하면 초대가 기다리고 있습니다.',
    inviteTargetLabel: '이름 또는 이메일',
    inviteTargetPlaceholder: '이름으로 찾거나 이메일 주소를 붙여넣으세요',
    candidateListLabel: '일치하는 계정',
    candidateLoading: '찾는 중…',
    ...
    sentAnnouncement: (target: string) => `${target} 로 초대를 보냈습니다.`,
  ...
  apiErrors: {
    ...
    invitation_target_ambiguous: '초대 대상은 목록에서 고른 사람 하나 또는 이메일 주소 하나여야 합니다.',
    invitation_target_unreachable: '그 계정은 초대 이메일을 받을 수 없습니다.',
  },
  workspace: {
    navLabel: '프로젝트 섹션',
    projectEyebrow: '프로젝트',
